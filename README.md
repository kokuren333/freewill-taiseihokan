...
参考URL・取り込んだ文書は自由意志入力の一部としてIndexedDBへ保存されます。URLはURLのまま、PDF・DOC・DOCXなどの文書は本文を抽出せず元ファイルのまま自由意志ZIPへ添付します。URLや文書内の指示はアプリでは実行せず、外部分析用の情報として扱います。文書はイベント日・記載日が分かる場合はその時系列を優先して解釈します。
