  ...
  return <div className="stack-lg audit-dashboard">
    ...
    <div className="dashboard-grid">
      ...
      <Panel><h2>最近の監査</h2><div className="period-table-wrap"><table className="history-table"><thead><tr><th>日時</th><th>主状態</th><th>副状態</th><th>主状態確率</th><th>質問</th></tr></thead><tbody>
        {ordered.slice(0, 12).map((entry) => <tr key={entry.id}><td>{new Date(entry.createdAt).toLocaleString('ja-JP', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</td><td>{entry.primaryStateLabel || model.states.find((s) => s.id === entry.primaryStateId)?.label || entry.primaryStateId}</td><td>{entry.secondaryStateLabel || (entry.secondaryStateId ? model.states.find((s) => s.id === entry.secondaryStateId)?.label || entry.secondaryStateId : '—')}</td><td>{percent(entry.probability)}</td><td>{entry.answers.length}</td></tr>)}
